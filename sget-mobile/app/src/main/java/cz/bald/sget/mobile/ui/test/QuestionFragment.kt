...
class QuestionFragment(
    ...
    private val question: Int,
    private val navIndex: Int
) : Fragment() {
    ...
    private fun createSectionView(inflater: LayoutInflater, container: ViewGroup?): View {
        ...
        frag.test_section_title.text = getString(
            R.string.test_section_title,
    ...
    private fun createTestQuestionView(inflater: LayoutInflater, container: ViewGroup?): View {
        ...
        frag.test_question_test_section_label.text = getString(
            R.string.test_section_title,
            test.sections[section].number
        )
        ...
        frag.test_question_test_answers.adapter = ArrayAdapter<String>(
            this.requireContext(),
    ...
    private fun createOpenQuestionView(inflater: LayoutInflater, container: ViewGroup?): View {
        ...
        frag.test_question_open_section_label.text = getString(
            R.string.test_section_title,
            test.sections[section].number
        )
        ...
        frag.test_question_open_answer.addTextChangedListener(
            OpenQuestionAnswerChangeListener(
                question
            )
        )
    ...
    private fun setupBackButton(button: Button) {
        if (section > 0 || question >= 0) {
            ...
            button.setOnClickListener {
                ...
                fcl.swapFragment(
                    QuestionFragment(test, nextSection, nextQuestion, NO_QUESTION),
                    false
                )
            }
        } else {
    ...
    private fun calculateAndSaveResult(): Result {
        ...
        val result = Result(
            0, test.setting, Date(), test.questionsCount, correct, points,
            test.maxPoints
        )
        test.result = result
        ...
        CoroutineScope(Dispatchers.IO).launch {
            context?.let { context ->
                SgetDatabase.getInstance(context).resultDao().insert(result)
            }
        }
